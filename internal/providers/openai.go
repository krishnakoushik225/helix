...
// openAI uses the same wire format as Ollama's OpenAI-compatible endpoint,
// so we reuse the same request/response shapes with local type aliases.
...
func (o *OpenAIProvider) HealthCheck(ctx context.Context) error {
	...
	if err != nil {
		return fmt.Errorf("openai: health check build request: %w", err)
	}
	o.setHeaders(req)
...
func (o *OpenAIProvider) CostPerInputToken(_ string) float64  { return 0.00000015 }
func (o *OpenAIProvider) CostPerOutputToken(_ string) float64 { return 0.0000006 }

func (o *OpenAIProvider) setHeaders(r *http.Request) {
